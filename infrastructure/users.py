...
def save_user(id_user, password, name, idrol, idcompanies):
    ...
    try:
        dba.execute_proc(proc_name=user_proc, args=user_args, assert_unique=True)
    ...
    for cid in idcompanies:
        ...
        try:
            dba.execute_proc(proc_name=usercom_proc, args=usercom_args, assert_unique=True)
        except dba.DbAdapterError as dbae:
            raise DatabaseError(dbae.get_message(),
...
def modify_user(id_user, password, name, idrol, idcompanies):
    # clear the user's companies first, so adding the ones received
    # gives the impression of deleting the missing ones and adding new ones
